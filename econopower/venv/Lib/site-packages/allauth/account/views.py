...
class EmailView(AjaxCapableProcessFormViewMixin, FormView):
    template_name = ("account/email_change." if app_settings.CHANGE_EMAIL else "account/email.") + app_settings.TEMPLATE_EXTENSION
    ...
    def _action_primary(self, request, *args, **kwargs):
        ...
        if email_address:
            ...
            if (
                ...
                get_adapter().add_message(request,messages.ERROR,"account/messages/unverified_primary_email.txt",)
            else:
                ...
                get_adapter().add_message(request,messages.SUCCESS,"account/messages/primary_email_set.txt",)
...
class PasswordChangeView(AjaxCapableProcessFormViewMixin, FormView):
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.user.is_anonymous:
            ...
            redirect_url = get_adapter(self.request).get_logout_redirect_url(self.request)
            return HttpResponseRedirect(redirect_url)
        elif not self.request.user.has_usable_password():
            return HttpResponseRedirect(reverse("account_set_password"))
        return super(PasswordChangeView, self).render_to_response(context, **response_kwargs)
...
class PasswordResetFromKeyView(AjaxCapableProcessFormViewMixin, LogoutFunctionalityMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        adapter.add_message(self.request,messages.SUCCESS,"account/messages/password_changed.txt",)
